...
// src/serverConfig.ts
function CreateServerConfig({ tailwind }) {
  return `
import type { ServerConfig } from "@bunpmjs/bunext/internal/types.ts";
${tailwind ? `import Tailwind from "@bunpmjs/bunext/external-plugins/tailwind.ts"` : ""};
...
// src/index.ts
var name = await esm_default3({
  message: "Project Name?"
});
var tailwind = await esm_default2({
...
if (tailwind) {
  ...
  await Bun.file(`${name}/config/server.ts`).write(CreateServerConfig({ tailwind }));
}
